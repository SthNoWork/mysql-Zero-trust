...
public class Server {
    ...
    static void connectDB() throws Exception {
        ...
        String url = "jdbc:mysql://" + cfg.getProperty("db.host") + ":" + cfg.getProperty("db.port") + "/" + schema
    ...
    static String mysqlGet(Map<String, String> params) throws Exception {
        ...
        if (params.containsKey("id")) {
            sql.append(" AND patient_id_hash=?");
            vals.add(sha256(params.get("id")));
        }
        if (params.containsKey("name")) {
            sql.append(" AND patient_name LIKE ?");
            vals.add("%" + params.get("name") + "%");
        }
        if (params.containsKey("doctor")) {
            sql.append(" AND doctor_name=?");
            vals.add(params.get("doctor"));
        }
        if (params.containsKey("nurse")) {
            sql.append(" AND nurse_name=?");
            vals.add(params.get("nurse"));
        }
        ...
        while (rs.next()) {
            ...
            json.append(",\"doctorEncryptedAesKey\":\"").append(b64(rs.getBytes("doctor_encrypted_aes_key")))
                    .append("\"");
            json.append(",\"nurseEncryptedAesKey\":\"").append(b64(rs.getBytes("nurse_encrypted_aes_key")))
                    .append("\"}");
        }
    ...
    static String supabaseGet(Map<String, String> params) throws Exception {
        ...
        if (params.containsKey("id"))
            url.append("&patient_id_hash=eq.").append(sha256(params.get("id")));
        if (params.containsKey("name"))
            url.append("&patient_name=like.*").append(params.get("name")).append("*");
        if (params.containsKey("doctor"))
            url.append("&doctor_name=eq.").append(params.get("doctor"));
    ...
    static void supabasePost(String body) throws Exception {
        ...
        if (d.get("patientDob") != null)
            json.append(",\"patient_dob\":\"").append(d.get("patientDob")).append("\"");
        json.append(",\"doctor_name\":\"").append(esc(d.get("doctorName"))).append("\"");
        json.append(",\"nurse_name\":\"").append(esc(d.get("nurseName"))).append("\"");
        if (d.get("encryptedSymptoms") != null)
            json.append(",\"encrypted_symptoms\":\"").append(d.get("encryptedSymptoms")).append("\"");
        if (d.get("encryptedDiagnosis") != null)
            json.append(",\"encrypted_diagnosis\":\"").append(d.get("encryptedDiagnosis")).append("\"");
        if (d.get("encryptedImages") != null)
            json.append(",\"encrypted_images\":\"").append(d.get("encryptedImages")).append("\"");
        if (d.get("encryptedVideos") != null)
            json.append(",\"encrypted_videos\":\"").append(d.get("encryptedVideos")).append("\"");
        if (d.get("encryptedAudios") != null)
            json.append(",\"encrypted_audios\":\"").append(d.get("encryptedAudios")).append("\"");
        if (d.get("doctorEncryptedAesKey") != null)
            json.append(",\"doctor_encrypted_aes_key\":\"").append(d.get("doctorEncryptedAesKey")).append("\"");
        if (d.get("nurseEncryptedAesKey") != null)
            json.append(",\"nurse_encrypted_aes_key\":\"").append(d.get("nurseEncryptedAesKey")).append("\"");
        json.append("}");
        HttpRequest req = HttpRequest.newBuilder()
                .uri(URI.create(cfg.getProperty("supabase.url") + "/rest/v1/" + TABLE))
                .header("apikey", cfg.getProperty("supabase.key"))
        ...
        if (resp.statusCode() >= 400)
            throw new Exception("Supabase error: " + resp.body());
    }
    ...
    static void supabasePut(String hid, String body) throws Exception {
        ...
        json.append("}");
        HttpRequest req = HttpRequest.newBuilder()
                .uri(URI.create(cfg.getProperty("supabase.url") + "/rest/v1/" + TABLE + "?record_index=eq." + hid))
                .header("apikey", cfg.getProperty("supabase.key"))
                .header("Authorization", "Bearer " + cfg.getProperty("supabase.key"))
                .header("Content-Type", "application/json")
    ...
    static void handleLogin(HttpExchange ex) throws IOException {
        ...
        try {
            ...
            if (role == null) {
                if (isSupabase) {
                    String url = cfg.getProperty("supabase.url") + "/rest/v1/users?username=eq." + d.get("username")
                            + "&password_hash=eq." + d.get("passwordHash") + "&select=role";
                    ...
                    PreparedStatement ps = db
                            .prepareStatement("SELECT role FROM users WHERE username=? AND password_hash=?");
            ...
            if (role != null)
                send(ex, 200, "{\"ok\":true,\"role\":\"" + role + "\"}");
            else
                send(ex, 401, "{\"error\":\"Invalid\"}");
        } catch (Exception e) {
            e.printStackTrace();
            send(ex, 500, "{\"error\":\"" + e.getMessage() + "\"}");
        }
    ...
    static void serveStatic(HttpExchange ex) throws IOException {
        ...
        if (Files.exists(f)) {
            ...
            ex.getResponseHeaders().set("Content-Type",
                    p.endsWith(".js") ? "application/javascript" : p.endsWith(".css") ? "text/css" : "text/html");
    ...
    static Map<String, String> parseJson(String j) {
        Map<String, String> m = new HashMap<>();
        for (String p : j.replaceAll("[{}\"]", "").split(",")) {
            String[] kv = p.split(":");
            if (kv.length == 2)
    ...
    static byte[] d64(String s) {
        return s == null || s.isEmpty() || s.equals("null") ? null : Base64.getDecoder().decode(s);
    }
